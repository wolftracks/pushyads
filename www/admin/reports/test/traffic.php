...
tracker_hit($db,$affiliate_member_id,TRACKER_AFFILIATE_PAGE,TRACKER_ID_AFFILIATE_GETPUSHY);
...
//-----------------------------------------------------------------------------------------------------------
 ...
 /* TEST For Duplicates - Same IP - Same Ad */
      ...
      tracker_hit($db,  'paw1200',   TRACKER_ELITE_BAR,   '1225');
      tracker_hit($db,  'paw1200',   TRACKER_ELITE_BAR,   '1225');
      tracker_hit($db,  'paw1200',   TRACKER_ELITE_BAR,   '1225');
      tracker_hit($db,  'paw1200',   TRACKER_ELITE_BAR,   '1225');
      tracker_hit($db,  'paw1200',   TRACKER_ELITE_BAR,   '1225');
      ...
      tracker_hit($db,  'paw1200',   TRACKER_ELITE_BAR,   '1225');
      tracker_hit($db,  'paw1200',   TRACKER_ELITE_BAR,   '1225');
      tracker_hit($db,  'paw1200',   TRACKER_ELITE_BAR,   '1226');


      tracker_hit($db,  'paw1200',   TRACKER_ELITE_BAR,   '1207');
      tracker_hit($db,  'wef1232b',  TRACKER_ELITE_BAR,   '1242');
      tracker_hit($db,  'hfe1201w',  TRACKER_ELITE_BAR,   '1213');
      tracker_hit($db,  'frg1223',   TRACKER_ELITE_BAR,   '1252');
      tracker_hit($db,  'hdn1208',   TRACKER_ELITE_BAR,   '1221');
      tracker_hit($db,  'mbr1217',   TRACKER_ELITE_BAR,   '1230');
      tracker_hit($db,  'wef1232b',  TRACKER_ELITE_BAR,   '1256');
 ...
 /* TEST For Duplicates - Same IP - Same Ad */
      tracker_click($db,  'wef1232b',   TRACKER_ELITE_BAR,   '1256');
      tracker_click($db,  'wef1232b',   TRACKER_ELITE_BAR,   '1256');
      tracker_click($db,  'wef1232b',   TRACKER_ELITE_BAR,   '1256');
      tracker_click($db,  'wef1232b',   TRACKER_ELITE_BAR,   '1256');
...
//-----------------------------------------------------------------------------------------------------------
      ...
      $tracker_id = "FirstPage";
 /* TEST For Duplicates - Same IP ADDRESS - Same WidgetName with Same Tracking ID */

      tracker_hit($db,$WidgetOwnwer,TRACKER_PUSHY_WIDGET, $WidgetName, $tracker_id);
      tracker_hit($db,$WidgetOwnwer,TRACKER_PUSHY_WIDGET, $WidgetName, $tracker_id);
      tracker_hit($db,$WidgetOwnwer,TRACKER_PUSHY_WIDGET, $WidgetName, $tracker_id);
      tracker_hit($db,$WidgetOwnwer,TRACKER_PUSHY_WIDGET, $WidgetName, $tracker_id);
      tracker_hit($db,$WidgetOwnwer,TRACKER_PUSHY_WIDGET, $WidgetName, $tracker_id);

      $tracker_id = "SecondPage";
      tracker_hit($db,$WidgetOwnwer,TRACKER_PUSHY_WIDGET, $WidgetName, $tracker_id);

      $tracker_id = "ThirdPage";
      tracker_hit($db,$WidgetOwnwer,TRACKER_PUSHY_WIDGET, $WidgetName, $tracker_id);


      tracker_hit($db,$WidgetOwnwer,TRACKER_PUSHY_WIDGET, $WidgetName, TRACKER_PUSHY_WIDGET_DEFAULT);
...
//-----------------------------------------------------------------------------------------------------------
      ...
      foreach($ads AS $ad_id => $ad_owner)
        {
          tracker_hit($db, $ad_owner,  TRACKER_PUSHY_ADS,  $ad_id);

          //--- Record "Widget Categories" ----
          for ($i=0; $i<count($categories); $i++)
            {
              tracker_hit($db, $WidgetOwner, TRACKER_WIDGET_CATEGORY, $WidgetKey, $categories[$i]);
            }
      ...
      //--- TEST FOR DUPLICATES
      $ad_owner='paw1200';
      tracker_hit($db, $ad_owner, TRACKER_PUSHY_ADS,  '1235');
      tracker_hit($db, $ad_owner, TRACKER_PUSHY_ADS,  '1235');
      tracker_hit($db, $ad_owner, TRACKER_PUSHY_ADS,  '1235');
      tracker_hit($db, $ad_owner, TRACKER_PUSHY_ADS,  '1235');
      tracker_hit($db, $ad_owner, TRACKER_PUSHY_ADS,  '1235');
      ...
      foreach($ads AS $ad_id => $ad_owner)
        {
          ...
          tracker_click($db, $ad_owner,  TRACKER_PUSHY_ADS,  $ad_id);

          //--- Record "Widget Categories" ----
          for ($i=0; $i<count($categories); $i++)
            {
              tracker_click($db, $WidgetOwner, TRACKER_WIDGET_CATEGORY, $WidgetKey, $categories[$i]);
            }
        }

      //--- TEST FOR DUPLICATES
      $ad_owner='paw1200';
      tracker_click($db, $ad_owner, TRACKER_PUSHY_ADS,  '1235');
      tracker_click($db, $ad_owner, TRACKER_PUSHY_ADS,  '1235');
      tracker_click($db, $ad_owner, TRACKER_PUSHY_ADS,  '1235');
      tracker_click($db, $ad_owner, TRACKER_PUSHY_ADS,  '1235');
      tracker_click($db, $ad_owner, TRACKER_PUSHY_ADS,  '1235');
