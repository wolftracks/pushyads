...
$DEFAULT_INTERVAL=300;         // "seconds"      between "Restart" for DOMAIN/IPAddress  - if interval is NOT Specified  ( 60 MINUTES )
...
if ($DUMP_HEADERS)
  {
    ...
    fputs($fp,sprintf("REFERER_URL:    %s\n",$referer_url));
...
if ($VERIFY_DOMAIN)
  {
    if (!startsWith($referer_domain,"pushyads.") && $REMOTE_ADDR != "127.0.0.1")
      {
...
if ($VERIFY_DOMAIN)
  {
    if (!$PUSHY_SITE && $REMOTE_ADDR != "127.0.0.1")
      {
...
//=========================================================================================================================
// Setting INTERVAL=0 says "always" show motion --- (for Owner Testing)
//=========================================================================================================================

$lasthit = tracker_last_hit($db, TRACKER_PUSHY_WIDGET, $WidgetOwner, buildWidgetKey($WidgetKey, $tracking_id), '');
if ($interval > 0)
  {
    $expired = tracker_test_set_hit($db,  $interval,  TRACKER_PUSHY_WIDGET, $WidgetOwner, buildWidgetKey($WidgetKey, $tracking_id), '');
    if (!$expired)
      {
...
//-- test-only --- $lasthit=0;
if ($lasthit == 0)  // (Does Not Exist in tracker_keys) First Hit Today - This User - This Widget
  {
     $today=getDateToday();
