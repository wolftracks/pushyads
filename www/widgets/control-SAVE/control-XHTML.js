...
function pushy_init()
  {
    ...
    pushy_container.style.width   =  _PUSHY_CONFIG.width  + 'px';
    pushy_container.style.height  =  _PUSHY_CONFIG.height + 'px';

    var tmpImage = document.createElement("img");
    tmpImage.setAttribute("src", _PUSHY_CONFIG.PUSHY_IMAGE_NAME_SHELL);
    tmpImage.setAttribute("border", "0");

    tmpImage.style.position = "relative";
    tmpImage.style.width    =  _PUSHY_CONFIG.width  + 'px';
    tmpImage.style.height   =  _PUSHY_CONFIG.height + 'px';
    tmpImage.style.top      = '0px';
    tmpImage.style.left     = '0px';

    pushy_container.appendChild(tmpImage);

    if (_PUSHY_CONFIG.GET_PUSHY_LINK)
      {
        var tmpImage = document.createElement("img");
        tmpImage.setAttribute("src", _PUSHY_CONFIG.PUSHY_IMAGE_GET_PUSHY)
        tmpImage.setAttribute("border", "0");
        tmpImage.style.position = "relative";

        if (_PUSHY_CONFIG.docType.isPresent)
          tmpImage.style.top      = '-36px';
        else
          tmpImage.style.top      = '-33px';

        tmpImage.style.left     = '90px';

        tmpImage.style.cursor = "pointer";
        tmpImage.onclick = _get_pushy_clicked_;
        tmpImage.onmouseover = function() { this.src = _PUSHY_CONFIG.PUSHY_IMAGE_GET_PUSHY_HOVER; }
        tmpImage.onmouseout  = function() { this.src = _PUSHY_CONFIG.PUSHY_IMAGE_GET_PUSHY; }
        pushy_container.appendChild(tmpImage);
      }


    /****
     <img    style="position:relative; top:0px;    left:0px;   width:320px;  height:395px;" src="http://pds1106.s3.amazonaws.com/widgets/images/pushy-shell-blank.png" border="0">
     <img    style="position:relative; top:-36px;  left:90px;  width:150px;  height:28px;"  src="http://pds1106.s3.amazonaws.com/widgets/images/get-pushy.png" onmouseover="this.style.cursor='hand';" border="0">
     <iframe style="position:relative; top:-319px; left:28px;  width:265px;  height:230px;" src="http://autoprospector.com" frameborder="0" scrolling="no"></iframe>
    ****/

    /**-------------------------------------------------------------**/
    var ifrm = document.createElement("iframe");
    ifrm.setAttribute("src", "http://autoprospector.com");
    ...
    if (_PUSHY_CONFIG.GET_PUSHY_LINK)
      {
        if (_PUSHY_CONFIG.docType.isPresent)
          ifrm.style.top    = "-319px";
        else
          ifrm.style.top    = "-311px";
        ifrm.style.left   = "28px";
      }
    else
      {
        if (_PUSHY_CONFIG.docType.isPresent)
          ifrm.style.top    = "-240px";
        else
          ifrm.style.top    = "-240px";
        ifrm.style.left   = "-25px";
      }
    /**-------------------------------------------------------------**/

    pushy_container.appendChild(ifrm);
