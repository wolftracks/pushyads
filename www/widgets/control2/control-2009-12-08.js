...
function pushy_setUserConfiguration()
  {
    var wzdd = new _WZDD_();
    wzdd.Int = function(d_x, d_y)
      {
        return isNaN(d_y = parseInt(d_x))? 0 : d_y;
      };
    wzdd.getWndW = function()
      {
        return wzdd.Int(
           (wzdd.db && !wzdd.op && !wzdd.w3c && wzdd.db.clientWidth)? wzdd.db.clientWidth
           : (window.innerWidth || 0)
        );
      };
    wzdd.getWndH = function()
      {
        return wzdd.Int(
          (wzdd.db && !wzdd.op && !wzdd.w3c && wzdd.db.clientHeight)? wzdd.db.clientHeight
          : (window.innerHeight || 0)
        );
      };
    wzdd.getScrollX = function()
      {
        return wzdd.Int(window.pageXOffset || (wzdd.db? wzdd.db.scrollLeft : 0));
      };
    wzdd.getScrollY = function()
      {
         return wzdd.Int(window.pageYOffset || (wzdd.db? wzdd.db.scrollTop : 0));
      };

    _PUSHY_CONFIG.wzdd=wzdd;
  }
...
function pushy_init()
  {
    ...
    pushy_home.style.width  = _PUSHY_CONFIG.widget_width  + 'px';
    pushy_home.style.height = _PUSHY_CONFIG.widget_height + 'px';
    ...
    var pushy_container = document.createElement("div");
    pushy_container.style.width   =  _PUSHY_CONFIG.widget_width  + 'px';
    pushy_container.style.height  =  _PUSHY_CONFIG.widget_height + 'px';


    /*-----------*/
    var pushy_widget = document.createElement("img");
    pushy_widget.style.position = "absolute";
    pushy_widget.setAttribute("id", "PUSHY_WIDGET");
    pushy_widget.setAttribute("src", _PUSHY_CONFIG.PUSHY_IMAGE_NAME_SHELL);
    pushy_widget.setAttribute("border", "0");
    pushy_widget.style.left   = pushyHome.left + 'px';
    pushy_widget.style.top    = pushyHome.top  + 'px';
    pushy_widget.style.width  = _PUSHY_CONFIG.widget_width  + 'px';
    pushy_widget.style.height = _PUSHY_CONFIG.widget_height + 'px';

    pushy_container.appendChild(pushy_widget);

    if (_PUSHY_CONFIG.GET_PUSHY_LINK)
      {
        var get_pushy = document.createElement("img");
        get_pushy.style.position = "absolute";
        get_pushy.setAttribute("id", "GET_PUSHY");
        get_pushy.setAttribute("src", _PUSHY_CONFIG.PUSHY_IMAGE_GET_PUSHY);
        get_pushy.setAttribute("border", "0");
        get_pushy.style.left   = (pushyHome.left+_PUSHY_CONFIG.get_pushy_offsetX) + 'px';
        get_pushy.style.top    = (pushyHome.top+_PUSHY_CONFIG.get_pushy_offsetY)  + 'px';
        get_pushy.style.width  = _PUSHY_CONFIG.get_pushy_width  + 'px';
        get_pushy.style.height = _PUSHY_CONFIG.get_pushy_height + 'px';

        get_pushy.style.cursor = "pointer";
        get_pushy.onclick = _get_pushy_clicked_;
        get_pushy.onmouseover = function() { this.src = _PUSHY_CONFIG.PUSHY_IMAGE_GET_PUSHY_HOVER; }
        get_pushy.onmouseout  = function() { this.src = _PUSHY_CONFIG.PUSHY_IMAGE_GET_PUSHY; }

        pushy_container.appendChild(get_pushy);
      }

    var pushy_scroller = document.createElement("iframe");
    pushy_scroller.style.position = "absolute";
    pushy_scroller.setAttribute("id", "PUSHY_SCROLLER");
    pushy_scroller.setAttribute("src", _PUSHY_CONFIG.DISPLAY_URL);
    pushy_scroller.setAttribute("frameBorder", "0");
    pushy_scroller.setAttribute("scrolling", "no");

    pushy_scroller.style.left   = (pushyHome.left+_PUSHY_CONFIG.pushy_scroller_offsetX) + 'px';
    pushy_scroller.style.top    = (pushyHome.top+_PUSHY_CONFIG.pushy_scroller_offsetY)  + 'px';
    pushy_scroller.style.width  = _PUSHY_CONFIG.pushy_scroller_width  + 'px';
    pushy_scroller.style.height = _PUSHY_CONFIG.pushy_scroller_height + 'px';

    /*-----------*/

    pushy_container.appendChild(pushy_scroller);
    ...
    shellImage.setAttribute("width",  _PUSHY_CONFIG.widget_width);
    shellImage.setAttribute("height", _PUSHY_CONFIG.widget_height);
    ...
    shellImage8Bit.setAttribute("width",  _PUSHY_CONFIG.widget_width);
    shellImage8Bit.setAttribute("height", _PUSHY_CONFIG.widget_height);
    ...
    spacerImage.setAttribute("width",  _PUSHY_CONFIG.widget_width);
    spacerImage.setAttribute("height", _PUSHY_CONFIG.widget_height);

    _pushy_.mybody          = mybody;
    _pushy_.mydiv           = mydiv;
    _pushy_.pushy_container = pushy_container;
    ...
    _pushy_.HoverVars     = {
                               ...
                               block:            false,
                               originalWidth:    _PUSHY_CONFIG.widget_width,
                               originalHeight:   _PUSHY_CONFIG.widget_height,
                               ftlObj:           null,
                               width:            0,
                               height:           0,
      ...
      if (_PUSHY_CONFIG.WidgetMotion == _PUSHY_CONFIG.WIDGET_MOTION_FLOAT)
        {
          ...
          blackScreen.setAttribute("width",  _PUSHY_CONFIG.widget_width);
          blackScreen.setAttribute("height", _PUSHY_CONFIG.widget_height);
      ...
      if (_PUSHY_CONFIG.WidgetMotion == _PUSHY_CONFIG.WIDGET_MOTION_BOUNCE)
        {
           ...
           pushyGuyImage.setAttribute("width",  _PUSHY_CONFIG.widget_width);
           pushyGuyImage.setAttribute("height", _PUSHY_CONFIG.widget_height);
            ...
            if (_pushy_.env.widget_posture_configuration == 0) //-- IF STATIC --  Transition is Deferred for Hover
              {
                ...
                morphImage.setAttribute("width",  _PUSHY_CONFIG.widget_width);
                morphImage.setAttribute("height", _PUSHY_CONFIG.widget_height);
...
function pushy_posture(pushyHome,widget)
  {
     ...
     if (widget.divChild)
       {
         if (widget.divChild != _pushy_.pushy_container)
           {
              _pushy_.mydiv.removeChild(widget.divChild);
              widget.divChild=null;
           }
     ...
     if (widget.bodyChild)
      ...
      if (_PUSHY_CONFIG.WidgetPosture == _PUSHY_CONFIG.WIDGET_POSTURE_HOVER)
        {
           // _pushy_.mydiv.appendChild(_pushy_.pushy_container);
           ...
           if (_pushy_.env.widget_transition_configuration == 3) // FADE IN
             {
                ...
                pushy_fade(image_id,millisec,from,to,function() { _pushy_.mydiv.removeChild(object); _pushy_.mydiv.appendChild(_pushy_.pushy_container); pushy_wiggle(pushyHome,widget); });
             }
          else
             {
                _pushy_.pushy_home.innerHTML="";
...
//         _pushy_.mydiv.removeChild(_pushy_.pushy_container);
//         _pushy_.mydiv.appendChild(_pushy_.pushy_container);
...
function pushy_hover_stayTopLeft()
  {
     if (!_pushy_.HoverVars.block)
       {
         if (_pushy_.HoverVars.horizontalOrigin=="fromright")
            _pushy_.HoverVars.ftlObj.x = _PUSHY_CONFIG.wzdd.getWndW() + _PUSHY_CONFIG.wzdd.getScrollX() - _pushy_.HoverVars.width - _pushy_.HoverVars.startX - _pushy_.HoverVars.horizontalOffset;
         else
            _pushy_.HoverVars.ftlObj.x = _PUSHY_CONFIG.wzdd.getScrollX() + _pushy_.HoverVars.startX + _pushy_.HoverVars.horizontalOffset;
         ...
         if (_pushy_.HoverVars.verticalOrigin=="fromtop")
           {
             var pY = _PUSHY_CONFIG.wzdd.getScrollY();
           ...
           {
             var pY =  _PUSHY_CONFIG.wzdd.getScrollY() + _PUSHY_CONFIG.wzdd.getWndH();
...
function pushy_hover_div(mydiv,verticalOrigin,verticalOffset,horizontalOrigin,horizontalOffset,transition)
  {
    ...
    if (_pushy_.HoverVars.transition > 0)
      ...
      {
         _pushy_.HoverVars.mydiv.appendChild(_pushy_.pushy_container);
         _pushy_.mybody.appendChild(_pushy_.HoverVars.mydiv);
      }
        ...
        if (pushy_absolute_value(LAST_Y - y) < 10)
          {
            ...
            var fStart=function() {
                  ...
                  setTimeout (function(){ _pushy_.HoverVars.mydiv.removeChild(morphImage); _pushy_.HoverVars.mydiv.appendChild(_pushy_.pushy_container);}, 2000);
            };
            ...
            var f=function()
               {
                  ...
                  pushy_fade(image_id,millisec,from,to, function(){ _pushy_.HoverVars.mydiv.removeChild(_pushy_.shellImage8Bit); _pushy_.HoverVars.mydiv.appendChild(_pushy_.pushy_container); });
               }
...
function pushy_page_width()
 {
   return _PUSHY_CONFIG.wzdd.getWndW();
 }
function pushy_page_height()
 {
   return _PUSHY_CONFIG.wzdd.getWndH();
 }
function pushy_pos_left()
 {
   return _PUSHY_CONFIG.wzdd.getScrollX();
 }
function pushy_pos_top()
 {
   return _PUSHY_CONFIG.wzdd.getScrollY();
 }
function pushy_pos_right()
 {
   return _PUSHY_CONFIG.wzdd.getScrollX() + _PUSHY_CONFIG.wzdd.getWndW();
 }
function pushy_pos_bottom()
 {
   return _PUSHY_CONFIG.wzdd.getScrollY() + _PUSHY_CONFIG.wzdd.getWndH();
 }
