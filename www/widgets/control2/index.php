...
if (count($_uri_tokens_) > 0)
 $file=trim($_uri_tokens_[count($_uri_tokens_)-1]);
...
//------------------------------------- TRACKING HERE ----- ( UNIQUENESS ??
if (isset($_qs_array_["tracker"]))
  {
    $tracker_id=strtolower($_qs_array_["tracker"]);
    tracker_hit($db,$member_id,TRACKER_PUSHY_WIDGET,$WidgetName,$tracker_id);
  }
else
  {
    tracker_hit($db,$member_id,TRACKER_PUSHY_WIDGET,$WidgetName,TRACKER_PUSHY_WIDGET_DEFAULT);
  }
//------------------------------------- TRACKING HERE -----
