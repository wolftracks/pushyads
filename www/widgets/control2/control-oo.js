var Pushy_Widget = function() {
  this.initialized    = false;
  this.wiggle_repeats = -1;


  this.initialize = function() {
    if (this.initialized) // Already initialized - don't do it again!
      return;
    this.initialized=true;

    this._setUserConfiguration();

    this.wiggle_interval = _PUSHY_CONFIG.wiggle_interval;      // interval between wiggles in seconds   = 0=No Wiggle
    this.wiggle_interval *= 1000;                              // in milliseconds
    this.wiggle_paused   = false;

    this.delay=_PUSHY_CONFIG.WidgetDelay;    // in seconds
    this.delay *= 1000;                      // in milliseconds
    this.pause=_PUSHY_CONFIG.WidgetPause;    // in seconds
    this.pause *= 1000;                      // in milliseconds
...
var pushyWidget  = new Pushy_Widget();
pushyWidget.CONFIG_={};
...
function _get_pushy_over_(obj)
  {
    var el=document.getElementById("Get_Pushy_Image");
    if ((el) && (el.src))
      el.setAttribute("src", _PUSHY_CONFIG.PUSHY_IMAGE_NAME_BOTTOM_HOVER);
  }

function _get_pushy_out_(obj)
  {
    var el=document.getElementById("Get_Pushy_Image");
    if ((el) && (el.src))
      el.setAttribute("src", _PUSHY_CONFIG.PUSHY_IMAGE_NAME_BOTTOM);
  }
...
function _pushy_GetScrollerLocation()
  {
    // alert("IN _pushy_GetScrollerLocation");
    if (typeof pushyGetScrollerLocation == 'function')
      {
        // alert("Yeah I See It");
        return pushyGetScrollerLocation(); // Optional External Function
      }
    // alert("Don't See It");
    return _PUSHY_CONFIG.DISPLAY_URL;
  }
...
Pushy_Widget.prototype.initialize = function()
  {
...
function pushy_init()
  {
       ...
       if (_PUSHY_CONFIG.GET_PUSHY_LINK)
         {
           ...
           tmpImage.setAttribute("id",  "Get_Pushy_Image");
           tmpImage.style.cursor = "pointer";
           tmpImage.onclick     = _get_pushy_clicked_;
           tmpImage.onmouseover = _get_pushy_over_;
           tmpImage.onmouseout  = _get_pushy_out_;
           /*------------------------------------------------------------------*/
         }
       else
         {
    ...
    ifrm.setAttribute("src", _pushy_GetScrollerLocation());
    ifrm.setAttribute("id", "PUSHY_SCROLLER");
...
//-- Public API - (No argument - Refresh Default Display URL)
function pushy_SetScrollerLocation(src) {
  if (_pushy_.ifrm)
    {
