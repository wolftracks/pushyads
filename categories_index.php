...
function tracker_getConfiguredCategorCounts($db,$activeWidgets)
  {
    if (count($activeWidgets)==0) return $activeWidgets;

    $sql  = "SELECT widget_categories from widget";

    foreach($activeWidgets AS $widget_key => $bool)
      {
...
function tracker_summary($db)
  {
    ...
    $activeWidgets           = array(); // Widget Cofiguration Key + TrackingId
    $activeWidgetKeys        = array(); // Widget Cofiguration Key
...
$activeWidgets           = $summary["activeWidgets"];
$activeWidgetKeys        = $summary["activeWidgetKeys"];
...
$format_header_widgets->setColor('yellow');       // ForeGround
...
foreach ($ProductCategories AS $cat => $ctitle)
  {
    ...
    if (isset($widgetCategoriesHit[$cat]))
       $h = count($widgetCategoriesHit[$cat]);
    if (isset($widgetCategoriesClicked[$cat]))
       $c = count($widgetCategoriesClicked[$cat]);
    if ($h==0 && $c==0)
      $widgets = $no_data;
    else
      $widgets = "($h/$c)";
...
$h=count($activeWidgetsHit);
$c=count($activeWidgetsClicked);
if ($h==0 && $c==0)
  $data=$no_data;
else
  $data="($h/$c)";
